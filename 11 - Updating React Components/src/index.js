...
shared.students = [
  ...
  new Student('Kristoffer', 'Kristoffersen', 'kristoffer.kristoffersen@ntnu.no', [
    shared.courses[0],
    shared.courses[2],
    shared.courses[3],
  ]),
  ...
  render() {
    return (
      <Card title="Systemutvikling 2 - Oppdatering av komponentvisninger i React">
        <br />
        Som i forrige øving skal du ha med påmeldte fag i student-visningen, og visning for fag med påmeldte studenter.
        <br />
        ...
        <br />
        Frivillig: prøv å lag andre gjenbrukbare komponenter i src/widgets.js-filen slik at du slipper å bruke
        bootstrap-klasser i src/index.js-filen. For eksempel kan følgende widgets lages i tillegg:
        <br />
        <br />
      ...
      <Card title="Add Course">
        <form>
          ...
          <Row>
            <Column width={2} />
            <Column>
              <Button.Green onClick={this.save}>Add</Button.Green>
            </Column>
          </Row>
        </form>
      </Card>
...
class CourseDetails extends Component<{ match: { params: { code: string } } }> {
  render() {
    ...
    let course_students = shared.students.filter((student) =>
      student.courses.find((scourse) => scourse.code == course.code)
    );
    return (
      <Card title="Details">
        <Row>
          ...
          <Row>
            <Column>
              <Button.Blue onClick={() => history.push('/courses/' + course.code + '/edit')}>edit</Button.Blue>
            </Column>
          </Row>
          <Column>
            <Button.Red onClick={this.delete}>delete</Button.Red>
          </Column>
        </Row>
        <br />
        <Row>
  ...
  delete() {
    ...
    let course = shared.courses.find((course) => course.code == this.props.match.params.code);
    shared.students.map(
      (student) => (student.courses = student.courses.filter((rcourse) => rcourse.code != course.code))
    );
...
class CourseEdit extends Component<{ match: { params: { code: string } } }> {
              ...
              <Column>
                {this.not_course_students.map((student) => (
                  <Row key={student.id}>
                    <Column width={2}>{student.firstName + ' ' + student.lastName}</Column>
                    <Column>
                      <Button.Green onClick={() => this.add(student)}>add</Button.Green>
                    </Column>
                  </Row>
                ))}
              </Column>
  ...
  mounted() {
    ...
    this.course_students = shared.students.filter((student) =>
      student.courses.find((scourse) => scourse.code == this.code)
    );
    this.not_course_students = shared.students.filter(
      (student) => !student.courses.find((scourse) => scourse.code == this.code)
    );
  }
    ...
    return (
      <Card title="Students">
        ...
        {shared.students.map((student) => (
          <Row key={student.id}>
            <Column width={2}>
              <Link to={'/students/' + student.id} title={student.firstName + ' ' + student.lastName} />
            </Column>
          </Row>
        ))}
      </Card>
      ...
      <Card title="Add student">
        <form>
          ...
          <Row>
            <Column width={2} />
            <Column>
              <Button.Green onClick={this.save}>Add Student</Button.Green>
            </Column>
          </Row>
          <br />
          <Card title="Courses">
                ...
                {this.courses.map((course) => (
                  <Row key={course.code}>
                    ...
                    <Column>
                      <Button.Red
                        onClick={() => {
                          this.courses = this.courses.filter((scourse) => scourse.code != course.code);
                        }}
                      >
                        remove
                      </Button.Red>
      ...
      <Card title="Details">
        <Row>
          ...
          <Row>
            <Column>
              {' '}
              <Button.Blue onClick={this.edit}>edit</Button.Blue>
            </Column>
            <Column>
              {' '}
              <Button.Red onClick={this.delete}>delete</Button.Red>
      ...
      <Card title="Edit">
        <form>
          ...
          <Row>
            <Column width={2}>First name</Column>
            <Column>
              <Form.input text={this.firstName} />
            </Column>
          </Row>
          <Row>
            <Column width={2}>Last name</Column>
