...
nettside.get("/GV", (req, res) => {
  pool.getConnection((err, connection) => {
    if (err) {
      ...
      connection.query(
        "select overskrift, innhold, tidspunkt, bilde, kategori, viktighet from artikkel where viktighet = 1 ",
        (err, rows) => {
          connection.release();
          if (err) {
            console.log(err);
...
nettside.get("/GV/artikler", (req, res) => {
  pool.getConnection((err, connection) => {
    if (err) {
      ...
      connection.query(
        "select id, overskrift, innhold, tidspunkt, bilde, kategori, viktighet from artikkel ",
        (err, rows) => {
          connection.release();
          if (err) {
            console.log(err);
...
nettside.post("/GV/artikler", (req, res) => {
  pool.getConnection((err, connection) => {
    if (err) {
      ...
      connection.query(
        "insert into artikkel (overskrift, innhold, bilde, kategori, viktighet) values (?, ?, ?, ?, ?)",
        val,
        (err) => {
          if (err) {
            console.log(err);
...
nettside.get("/GV/:kategori", (req, res) => {
  pool.getConnection((err, connection) => {
    if (err) {
      ...
      connection.query(
        "select overskrift, innhold, tidspunkt, bilde, kategori, viktighet from artikkel where kategori = ? ",
        val,
        (err, rows) => {
          connection.release();
          if (err) {
...
nettside.get("/GV/artikkel/:id", (req, res) => {
  pool.getConnection((err, connection) => {
    if (err) {
      ...
      connection.query(
        "select overskrift, innhold, tidspunkt, bilde, kategori, viktighet from artikkel where id = ?",
        val,
        (err, row) => {
          connection.release();
          if (err) {
...
nettside.put("/GV/artikkel/:id", (req, res) => {
  pool.getConnection((err, connection) => {
    if (err) {
      ...
      connection.query(
        "update artikkel set overskrift = ?, innhold = ?, bilde = ?, kategori = ?, viktighet = ? where id = ?",
        val,
        (err) => {
          if (err) {
            console.log(err);
...
nettside.delete("/GV/artikkel/:id", (req, res) => {
  pool.getConnection((err, connection) => {
    if (err) {
      ...
      let val = [req.params.id];
      connection.query("delete from artikkel where id = ?", val, (err) => {
        if (err) {
          console.log(err);
          res.status(500);
