...
app.post("/login", async (req, res) => {
  ...
  let credentials = await hashPassword.hashPassword(
    req.body.password,
...
app.get("/validate/email/:email", (req, res) => {
  return db.getUserByEmail(req.params.email).then((user) => {
    user !== null
      ? db
          .getSaltByEmail(user.email)
          .then((salt) => res.send(salt[0].dataValues.salt !== null))
      : res.send(false);
  });
});
...
app.post("/users", (req, res) => {
  return db
    .getUserByEmailOrUsername(req.body.email, req.body.username)
    .then((users) => users.map((user) => user.dataValues))
    .then(async (users) => {
      if (users.length !== 0) {
        users = await users.filter(async (user) => {
          await db.getSaltByEmail(user.email).then((salt) => {
...
app.get("/users", (req, res) => {
  return db
    .getAllUsers()
    .then((users) =>
      users ? res.status(201).send(users) : res.sendStatus(400)
    );
});
...
app.get("/users/:userId", (req, res) => {
  return db
    ...
    .then((user) =>
      user !== null ? res.status(201).send(user) : res.sendStatus(400)
    );
});
...
app.post("/auth/events", (req, res) => {
  if (req.body.imageUrl && req.body.imageUrl.includes("base64")) {
    let name = "image.png";
    filehandler.uploadToCloud(req.body.imageUrl, name, true).then((urldata) => {
      req.body.imageUrl = urldata.url;
      db.createEvent(req.body).then((response) =>
        response.insertId ? res.status(201).send(response) : res.sendStatus(400)
      );
    });
  } else {
    db.createEvent(req.body).then((response) =>
  ...
  db.deleteOldEvents()
    ...
    .then(() => {
      return db
        .getAllEvents()
        .then((events) =>
          events !== null ? res.status(201).send(events) : res.sendStatus(400)
        );
    })
    .catch((error) => console.error(error));
});
...
app.get("/events/eventDetails/:eventId", (req, res) => {
  return db
    .getEventByEventId(req.params.eventId)
    .then((event) =>
      event !== {} ? res.status(201).send(event) : res.sendStatus(404)
    );
});
...
app.get("/auth/events/users/:userId", (req, res) => {
  ...
  if (decoded.userId == req.params.userId) {
    return db
      .getEventsByOrganizerId(decoded.userId)
...
app.get("/myevents/users/:userId/", (req, res) => {
  return db
    .getMyEventsByUserId(req.params.userId)
    .then((events) => res.send(events))
    .catch((error) => console.error(error));
});
...
app.put("/auth/events/:eventId", (req, res) => {
  ...
  if (req.body.imageUrl && req.body.imageUrl.includes("base64")) {
    db.getEventByEventId(req.params.eventId).then((item) => {
      filehandler.deleteFromCloud(
        filehandler.getNameFromUrl(item.imageUrl, true),
        true
      );
      return filehandler
        .uploadToCloud(req.body.imageUrl, "img.png", true, false)
        .then((data) => {
...
app.delete("/auth/events/:eventId", (req, res) => {
  db.getEventByEventId(req.params.eventId).then((item) => {
    ...
    return db
      ...
      .then((deleteOk) => (deleteOk ? res.sendStatus(201) : res.status(400)))
      .catch((err) => res.status(400));
  });
});
...
app.post("/auth/events/:eventId/personnel", (req, res) => {
  return db
    .addPersonnel(req.body)
    .then((insertOk) =>
      insertOk ? res.status(201).send(true) : res.sendStatus(503).send(false)
    );
});
...
app.put("/auth/events/:eventId/personnel", (req, res) => {
  return db
    .updatePersonnel(req.body)
    .then((updateOk) =>
      updateOk ? res.status(201).send(true) : res.status(404).send(false)
    );
});
...
app.delete("/auth/events/:eventId/personnel/:personnelId", (req, res) => {
  ...
  return db
    ...
    .then((deleteOk) => (deleteOk ? res.sendStatus(201) : res.sendStatus(503)));
});
...
app.get("/auth/events/:eventId/personnel", (req, res) => {
  ...
  return db
    ...
    .then((personnel) =>
      personnel !== null ? res.status(201).send(personnel) : res.sendStatus(404)
    );
});
...
app.post("/auth/events/:eventId/tickets", (req, res) => {
  return db
    .addTickets(req.body)
    .then((insertOk) =>
      insertOk ? res.status(201).send(true) : res.status(503).send(false)
    );
});
...
app.delete("/auth/events/:eventId/tickets/:type", (req, res) => {
  ...
  return db
    ...
    .then((deleteOk) =>
      deleteOk ? res.status(201).send(true) : res.status(400).send(false)
    );
});
...
app.get("/events/:eventId/tickets", (req, res) => {
  ...
  return db
    .getTickets(eventId)
    .then((tickets) =>
      tickets !== null ? res.status(201).send(tickets) : res.sendStatus(404)
    );
});
...
app.post("/auth/events/:eventId/gigs", (req, res) => {
  if (req.body.contract.data && req.body.contract.data.includes("base64")) {
    filehandler
      .uploadToCloud(req.body.contract.data, req.body.contract.name, false)
      .then((file) => {
        req.body.contract.data = file.url;
        req.body.contract.name = file.name;
        db.addGig(req.body).then((insertOk) =>
          insertOk ? res.status(201).send(insertOk) : res.sendStatus(503)
        );
      });
  } else {
    db.addGig(req.body).then((insertOk) =>
...
app.delete("/auth/events/:eventId/gigs/:artistId", (req, res) => {
  ...
  db.deleteGig(eventId, artistId).then((deleteOk) =>
    deleteOk ? res.status(201).send(true) : res.sendStatus(503)
  );
});
...
app.get("/auth/events/:eventId/gigs", (req, res) => {
  ...
  return db
    .getGigs(eventId)
    .then((gigs) =>
      gigs !== null ? res.status(201).send(gigs) : res.sendStatus(400)
    );
});
...
app.get("/auth/events/:eventId/gigs/:artistId", (req, res) => {
  ...
  db.getContract(eventId, artistId).then((contract) => {
    filehandler
      .downloadFromCloud(contract.name)
      .then((dataString) => {
 ...
 *  @return {json} {jwt: token}
 */
app.delete("/auth/events/:eventId/gigs/:artistId/rider/:item", (req, res) => {
  let eventId = decodeURIComponent(req.params.eventId);
  let artistId = decodeURIComponent(req.params.artistId);
  let item = decodeURIComponent(req.params.item);
  db.deleteRiderItem(eventId, artistId, item).then((deleteOk) => {
    return deleteOk ? res.status(201).send(true) : res.sendStatus(400);
  });
});
...
app.put("/auth/events/:eventId/gigs/:artistId/rider", (req, res) => {
  db.updateRiderItems(req.body).then((updateOk) =>
    updateOk ? res.status(201).send(true) : res.status(401).send(false)
  );
});
...
app.get("/auth/events/:eventId/gigs/:artistId/rider", (req, res) => {
  let eventId = decodeURIComponent(req.params.eventId);
  let artistId = decodeURIComponent(req.params.artistId);
  db.getRiderItems(eventId, artistId).then((riderItems) =>
    riderItems ? res.status(201).send(riderItems) : res.status(404).send([])
  );
});
