...
class Dao {
  ...
  deleteUser(userId) {
    return model.UserModel.update(
      ...
      .then(() => {
        return this.getEventsByOrganizerId(userId).then((events) => {
          return Promise.all(
            events.map((event) => {
                ...
                return this.getGigs(event.eventId)
                  ...
                  .then((gigs) => {
                    return this.getPersonnel(event.eventId)
                      .then((personnel) =>
                        personnel.map((person) => person.dataValues.personnelId)
                      )
                      .then((personnel) => {
                        ...
                        return model.UserModel.findAll({
                          where: { userId: { [op.in]: Array.from(set) } },
                        })
                          .then((users) =>
                            users.map((user) => user.dataValues.email)
                          )
                          .then((users) => {
                            let email = {
                              from: mailProps.username,
  ...
  deleteEvent(eventId) {
    if (!isCI && !test) {
      return this.getEventByEventId(eventId).then((event) => {
        return this.getUserById(event.organizerId).then((user) => {
                  ...
                  return model.UserModel.findAll({
                    ...
                    .then((users) => {
                      ...
                      return this.deleteGigs(eventId)
                        .then(() => {
                          return model.TicketModel.destroy({
                            where: { eventId: eventId },
                          }).then(() => {
                            return model.PersonnelModel.destroy({
                              where: { eventId: eventId },
                            }).then(() => {
                              return model.EventModel.destroy({
                                where: { eventId: eventId },
                              }).then((res) => {
                                mail.sendMail(email);
                                return res;
  ...
  deleteOldEvents() {
    return model.EventModel.findAll({
      where: { endTime: { [Op.lt]: moment().subtract(90, "days").toDate() } },
    });
  }
   ...
   */
  getAllEvents() {
    return model.EventModel.findAll({ order: [["startTime", "ASC"]] }).catch(
      (error) => {
        console.error(error);
        return [];
      }
  ...
  async getMyEventsByUserId(userId) {
    let personnelEvents = await model.PersonnelModel.findAll({
    ...
    let artistEvents = await model.GigModel.findAll({
    ...
    return model.EventModel.findAll({
      where: {
        [Op.or]: [{ eventId: artistEvents }, { eventId: personnelEvents }],
      },
    }).catch((error) => {
      console.error(error);
      return [];
  ...
  getEventByEventId(eventId) {
    return model.EventModel.findOne({ where: { eventId: eventId } }).catch(
      (error) => {
        console.error(error);
        return {};
      }
  ...
  addPersonnel(personnel) {
    return model.PersonnelModel.bulkCreate(personnel)
      .then((response) => {
        if (!isCI && !test) {
          this.getEventByEventId(response[0].eventId).then((event) => {
            this.getPersonnel(event.eventId).then((person) => {
              person.map((pers) => {
                let email = {
                  from: mailProps.username,
                  to: pers.user.email,
  ...
  addGig(gig) {
    return model.FileModel.create({
      ...
      return model.GigModel.create({
        ...
        .then((response) => {
          if (!isCI && !test) {
            this.getUserById(gig.artistId).then((user) => {
              this.getEventByEventId(gig.eventId).then((event) => {
                let email = {
                  to: user.email,
                  from: mailProps.username,
  ...
  deleteGig(eventId, artistId) {
    return this.getContractId(eventId, artistId)
      .then((contractId) => {
        return model.FileModel.findByPk(contractId.dataValues.contract).then(
          (contract) => {
            filehandler.deleteFromCloud(contract.name, false);
            return model.FileModel.destroy({
              where: { fileId: contractId },
            }).then(() => {
  ...
  updateRiderItems(riderItems) {
    return Promise.all(
      riderItems.map((riderItem) =>
        model.RiderModel.update(
          {
      ...
      if (!isCI && !test) {
        return this.getUserById(riderItems[0].artistId).then((user) => {
          return model.EventModel.findOne({
            where: { eventId: riderItems[0].eventId },
            include: [{ model: model.UserModel }],
          }).then((event) => {
  ...
  deleteRiderItem(eventId, artistId, item) {
    return model.RiderModel.destroy({
      where: { eventId: eventId, artistId: artistId, item: item },
    })
      .then(() => true)
      .catch((error) => {
        console.error(error);
  ...
  getRiderItems(eventId, artistId) {
    return model.RiderModel.findAll({
      where: { eventId: eventId, artistId: artistId },
    }).catch((error) => {
      console.error(error);
      return [];
