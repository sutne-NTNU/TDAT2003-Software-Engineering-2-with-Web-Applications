...
describe("Users", () => {
  ...
  it("getSaltByEmail", (done) => {
    db.getSaltByEmail("steffen@mail.com").then((salt) => {
      expect(salt[0].dataValues).toEqual({
  ...
  it("getUserByEmailOrUsername", (done) => {
    db.getUserByEmailOrUsername("steffen@mail.com", "Marius T").then(
      (users) => {
        expect([
          {
...
describe("Events", () => {
  ...
  it("getEventsByOrganizerId", (done) => {
    db.getEventsByOrganizerId(2).then((events) => {
  ...
  it("getMyEventsByUserId", (done) => {
    db.getMyEventsByUserId(2).then((events) => {
      expect(
        events
          .map((event) => event.toJSON())
          .map((event) => ({
  ...
  it("getEventsByEventId", (done) => {
    db.getEventByEventId(1).then((event) => {
      expect({
        eventId: event.eventId,
        organizerId: event.organizerId,
        eventName: event.eventName,
...
describe("Personnel", () => {
  ...
  it("getPersonnel", (done) => {
    db.getPersonnel(4).then((personnel) => {
      ...
      expect(
        personnel
          .map((personnel) => personnel.toJSON())
          .map((personnel) => ({
