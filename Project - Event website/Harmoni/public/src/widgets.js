...
export class EventInfo extends Component {
  ...
  getAgeLimitInfo(age_limit) {
    if (age_limit !== 0) {
      ...
      return <div className="font-weight-bold">Tillat For Alle</div>;
    }
  }
        ...
        <Row>
          <Col>
            <small className="text-muted">
              {" "}
              Publisert {this.formatTimeDate(this.props.uploaded)}
            </small>
          </Col>
          <Col>
            <Button
              href={"/endre-arrangement/" + this.props.link}
              variant="primary"
              size="sm"
            >
              Rediger Arragement
  ...
  render() {
    return (
      <Col md={12} lg={4} className={"mt-2 mb-2"} style={{ cursor: "pointer" }}>
        <Card
          onMouseEnter={() => this.setState({ hoverCss: "shadow-lg" })}
        ...
        >
          <Card className="border-0">
            <div style={{ height: "13em", overflow: "hidden" }}>
              ...
              <Card.ImgOverlay>
                <Card.Title>
                  <h1 className="imageText">{this.getImageOverlayTxt()}</h1>
                </Card.Title>
              </Card.ImgOverlay>
            </div>
          </Card>
          <Card.Body>
            <div className="eventinfo">
              ...
              <Card.Text>
                ...
                <div className="font-weight-bold">Fra</div>
                {this.props.start_date}
                <div className="font-weight-bold">Til</div>
                {this.props.end_date}
              </Card.Text>
            </div>
          </Card.Body>
          <Card.Footer>{this.getCardFooter(this.props.myEvent)}</Card.Footer>
        </Card>
      </Col>
    );
  }
...
export class DownloadWidget extends Component {
  ...
  download() {
    service.downloadContract(this.event, this.artist).then((response) => {
...
export class MailForm extends Component {
  constructor(props) {
    super(props);
    this.state = {
      recipientString: this.getRecipentString(props)
        ? this.getRecipentString(props)
        : "",
          ...
          <div className={""}>
            <Card className="border-0 m-sm-5 mt-4 p-sm-4">
              <div>
                ...
                {this.state.toggle ? (
                  this.toggleForm(this.state.toggle)
                ) : (
      ...
      return (
        <Form className={"text-left"}>
          ...
          <Form.Control
            ...
            value={this.state.recipientString}
            placeholder={"mottaker1@mail.com, mottaker2@mail.com"}
            rows="1"
            style={{ display: "flex" }}
          />
