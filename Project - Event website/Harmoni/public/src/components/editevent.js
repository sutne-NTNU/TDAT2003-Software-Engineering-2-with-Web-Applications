...
export default function EditEvent() {
  ...
  const [maxTime, setMaxTime] = useState(
    moment("23:59", "HH:mm").format("HH:mm")
  );
  ...
  useEffect(() => {
    service
      .getEventByEventId(match.params.id)
      .then((event) => {
        if (event.organizerId != jwt.decode(authService.getToken()).userId)
          history.push("/arrangement/" + event.eventId);
        ...
        service
          .getPersonnel(event.eventId)
          .then((p) => handlePersonnelFromDatabase(p))
          .catch((err) => console.error(err));
        service
          .getTicketToEvent(event.eventId)
          .then((t) => handleTicketsFromDB(t))
          .catch((err) => console.error(err));
        service
          .getGigs(event.eventId)
          .then((g) => handleGigsFromDatabase(g))
          .catch((err) => console.error(err));
      })
      .catch((err) => console.error(err));
  }, []);
    ...
    if (
      ...
      gigsNewByEmail.map((gig) => {
        if (gig.contract === null) {
          errmsg +=
            "  [ Kontrakt for " +
            (gig.user.email ? gig.user.email : "ny artist") +
            " mangler ] ";
        }
      });
                    ...
                    <Dropdown onSelect={(event) => handleGigAdd(event)}>
                      ...
                      >
                        {users
                          .filter((user) => {
                            let exists = false;
                            if (gigsOld.some((a) => a.artistId === user.userId))
                              exists = true;
                            if (gigsNew.some((a) => a.artistId === user.userId))
                              exists = true;
                            return !exists;
                          })
                          .map((user) => (
            ...
            <Row className={"mt-4 text-center"}>
              ...
              {error ? (
                <Alert
                  ...
                  variant={errorType}
                >
                  <Alert.Heading>Vent nå litt!</Alert.Heading>
                  <p>{error}</p>
                </Alert>
              ) : (
  ...
  function renderImagePreview() {
    ...
    return (
      <div className="imgPreviewFrame">
        ...
        <img src={imageUrl} height={"100%"} alt={"Failed to load image"} />
      </div>
    );
  }
  ...
  function renderArtists() {
    if (gigsOld.length + gigsNew.length + gigsNewByEmail.length === 0)
      return null;
    return (
      <Form.Group as={Col} sm={"12"}>
        <Card>
          <Card.Title>
            <h2 className="text-center">
              {gigsOld.length + gigsNew.length + gigsNewByEmail.length > 1
                ? "Artister"
                : "Artist"}
            </h2>
          </Card.Title>
            ...
            <Row>
              ...
              <Form.Group as={Col} sm={"3"}>
                <UploadWidget
                  ...
                  onChange={(event) =>
                    handleContractUpload(event.target.files[0], gig)
                  }
                />
              </Form.Group>
  ...
  async function updateGigs() {
    return new Promise((resolve, reject) => {
      ...
      if (Array.isArray(gigsRemove) && gigsRemove.length > 0) {
        promises.push(
          gigsRemove.map((gig) =>
            service.deleteGig(gig).catch((error) => reject(error))
          )
        );
      }
      if (Array.isArray(gigsNew) && gigsNew.length > 0) {
        promises.push(
          gigsNew.map((gig) =>
            service.addGig(gig).catch((error) => reject(error))
          )
        );
      }
                  ...
                  <Row>
                    ...
                    <Col sm={6}>
                      <Form.Control
                        ...
                        value={p.role}
                        onChange={(event) =>
                          handlePersonnelRoleChange(event, p)
                        }
                      />
                    </Col>
                    <Col sm={1}>
                      <Button
                        type="button"
                        variant={"danger"}
                        onClick={() => handlePersonnelRemoval(p)}
                      >
                        X
                      </Button>
                    </Col>
  ...
  async function updatePersonnel() {
    return new Promise((resolve, reject) => {
      ...
      if (Array.isArray(personnelRemove) && personnelRemove.length > 0) {
        promises.push(
          personnelRemove.map((personnel) =>
            service
              .deletePersonnel(eventId, personnel.personnelId)
              .catch((error) => reject(error))
          )
        );
      }
      ...
      if (Array.isArray(personnelUpdate) && personnelUpdate.length > 0) {
        promises.push(
          service
            .updatePersonnel(personnelUpdate)
            .catch((error) => reject(error))
        );
      }
      ...
      if (Array.isArray(personnelAdd) && personnelAdd.length > 0) {
        promises.push(
          service.addPersonnel(personnelAdd).catch((error) => reject(error))
        );
      }
  ...
  function handleTicketsFromDB(event_ticketsFromDB) {
    let newTickets = event_ticketsFromDB.map(
      (t) => new Ticket(t.eventId, t.type, t.price, t.amount)
    );
  ...
  async function updateTickets() {
    return new Promise((resolve, reject) => {
      ...
      if (Array.isArray(deletedTickets) && deletedTickets.length > 0) {
        promises.push(
          deletedTickets.map((ticket) =>
            service.deleteTicket(ticket).catch((error) => reject(error))
          )
        );
      }
      ...
      if (Array.isArray(updatedTickets) && updatedTickets.length > 0) {
        promises.push(
          service.updateTicket(updatedTickets).catch((error) => reject(error))
        );
      }
      ...
      if (Array.isArray(addedTickets) && addedTickets.length > 0) {
        promises.push(
          service.addTickets(addedTickets).catch((error) => reject(error))
        );
      }
