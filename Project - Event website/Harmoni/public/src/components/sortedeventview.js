...
export class SortedEventView extends Component {
  ...
  getFilteredEvents = (events) => {
    ...
    return events.filter(
      (event) => event.eventName.match(regex) || event.description.match(regex)
    );
  };

  getActiveState = (option) => {
    if (option === "asc" || option === "desc")
      return option === this.state.order ? "active" : "";
    return option === this.state.sortingOption ? "active" : "";
  };

  sortEvents = (sortingOption, order) => {
    this.setState({
      events: this.state.events.sort(this.compareValues(sortingOption, order)),
    });
  };
  ...
  handleFilterOption(filter) {
    ...
    switch (filter) {
      case "ChildFriendly":
        this.setState({
          events: this.state.events.filter((event) => event.ageLimit < 7),
        });
        this.setState({ childFriendly: "active" });
        break;
      case "HideCancelled":
        this.setState({
          events: this.state.events.filter((event) => !event.cancelled),
        });
  ...
  handleEvents = (events) => {
    ...
    this.setState({
      ...
      filteredEvents: events.filter((event) =>
        moment(now).isBefore(event.endTime)
      ),
      eventsBackup: events,
    });
  };
          ...
          <Navbar className={"shadow-sm mt-2"} bg="light" expand="lg">
            ...
            <Navbar.Collapse id="basic-navbar-nav">
              <Nav className="mr-auto">
                ...
                >
                  <NavDropdown.Item
                    className={this.getActiveState("ageLimit")}
                    onClick={() => this.handleSortingOption("ageLimit")}
                  >
                    Aldersgrense{" "}
                  </NavDropdown.Item>
                  <NavDropdown.Item
                    className={this.getActiveState("createdAt")}
                    onClick={() => this.handleSortingOption("createdAt")}
                  >
                    Publisert
                  </NavDropdown.Item>
                  <NavDropdown.Item
                    className={this.getActiveState("address")}
                    onClick={() => this.handleSortingOption("address")}
                  >
                    Adresse
                  </NavDropdown.Item>
                  <NavDropdown.Item
                    className={this.getActiveState("startTime")}
                    onClick={() => this.handleSortingOption("startTime")}
                  >
                    Dato
                  </NavDropdown.Item>
                </NavDropdown>
                <NavDropdown
                  className={this.state.filterActive}
                  title="Filtere"
                  id="basic-nav-dropdown"
                >
                  <Dropdown.Item
                    className={this.state.childFriendly}
                    onClick={() => this.handleFilterOption("ChildFriendly")}
                  >
                    Barnevennelig (6 år){" "}
                  </Dropdown.Item>
                  <Dropdown.Item
                    className={this.state.hideCancelled}
                    onClick={() => this.handleFilterOption("HideCancelled")}
                  >
                    Skjul kansellerte arrangementer
                  </Dropdown.Item>
                  <Dropdown.Item
                    className={this.state.showOld}
                ...
                <NavItem>
                  <Button
                    className={this.getActiveState("asc")}
                    onClick={() => this.handleOrder("asc")}
                    variant={"light"}
                  >
                    {this.state.ascDescription}
                  </Button>
                </NavItem>
                <NavItem>
                  <Button
                    className={this.getActiveState("desc")}
                    onClick={() => this.handleOrder("desc")}
                    variant={"light"}
                  >
                    {this.state.descDescription}
                  </Button>
                </NavItem>
                <NavItem>
                  <Button
