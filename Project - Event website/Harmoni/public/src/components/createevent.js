                    ...
                    <Dropdown onSelect={(event) => handleGigAdd(event)}>
                      ...
                      >
                        {users
                          .filter(
                            (user) =>
                              !gigsNew.some((a) => a.artistId === user.userId)
                          )
                          .map((user) => (
        ...
        <Card>
          <Card.Title>
            <h2 className="text-center">
              {gigsNew.length + gigsNewByEmail.length > 1
                ? "Artister"
                : "Artist"}
            </h2>
          </Card.Title>
    ...
    return new Promise((resolve, reject) => {
      ...
      if (Array.isArray(gigsNew) && gigsNew.length > 0) {
        promises.push(
          gigsNew.map((gig) =>
            service
              .addGig(new Gig(eventId, gig.artistId, gig.contract))
      ...
      if (Array.isArray(personnel) && personnel.length > 0) {
        promises.push(
          service
            .addPersonnel(
              personnel.map(
                (p) => new Personnel(p.personnelId, eventId, p.role)
              )
            )
            .catch((error) => reject(error))
        );
    ...
    return new Promise((resolve, reject) => {
      ...
      if (Array.isArray(tickets) && tickets.length > 0) {
        promises.push(
          service
            .addTickets(
              tickets.map((t) => new Ticket(eventId, t.type, t.price, t.amount))
            )
            .catch((error) => reject(error))
        );
      }
