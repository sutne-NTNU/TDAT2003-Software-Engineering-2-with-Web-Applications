...
export class RiderPage extends Component {
  ...
  saveItem() {
    ...
    if (
      this.state.newItems.some(
        (item) => this.state.currentItem.trim() === item.item
      ) ||
      this.state.oldItems.some(
        (item) => this.state.currentItem.trim() === item.item
      )
    ) {
  ...
  saveRider() {
    ...
    let promises = [];
    if (
      Array.isArray(this.state.deleteItems) &&
      this.state.deleteItems.length > 0
    ) {
      promises.push(
        this.state.deleteItems.map((item) =>
          service.deleteRiderItem(item).catch((error) => console.error(error))
        )
      );
    }
            ...
            <Card className={"m-4"}>
              <h1 className="font-weight-bold text-center mt-4">
                {this.state.artist.username +
                  " har ikke lastet opp en rider enda"}
              </h1>
              <div className="text-center">
                <Button
                  variant={"primary"}
            ...
            <Card className="p-5">
              ...
              <Form.Group>
                <div className="text-center">
                  <Button
                    variant="success"
                    onClick={() => this.saveConfirmedItems()}
                  >
                    Lagre
                  </Button>
                  <Button
        ...
        <Container>
          <Card className="p-5">
            <h1 className="font-weight-bold text-center">
              {"Rider for " + this.state.artist.username}
            </h1>
            <Form.Group>
              <ListGroup>
                <React.Fragment>
