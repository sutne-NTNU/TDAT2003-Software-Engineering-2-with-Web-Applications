...
export class EventPage extends Component {
          ...
          <Container className={"c-md"}>
            <Card className="p-2">
              ...
              <div className="p-sm-4">
                <h1 className="display-4 text-center m-4 text-body">
                  ...
                  <Col>
                    <ListGroup variant="flush">
                      <ListGroup.Item>
                        <h6>
                          <b>Fra:</b>{" "}
                          {this.formatTime(this.currentEvent.startTime)}
                        </h6>
                      </ListGroup.Item>
                      <ListGroup.Item>
                        <h6>
                          <b>Til:</b>{" "}
                          {this.formatTime(this.currentEvent.endTime)}
                        </h6>
                      </ListGroup.Item>
                      <ListGroup.Item>{this.RenderAgeLimit()}</ListGroup.Item>
                      <ListGroup.Item>
                        <h6>
                          <b>Adresse:</b> {this.currentEvent.address}
                        </h6>
                        <h6>
                          <b>By:</b> {this.currentEvent.city}
                        </h6>
                        <Button
                          type="button"
                          size="sm"
      ...
      return (
        <div>
          <Button
            ...
            size="sm"
            href={
              "/arrangement/" + this.currentEvent.eventId + "/rider/" + artistId
            }
          >
            Vis Rider
          </Button>
  ...
  ShowArtist() {
    if (
      ...
      (this.isPersonnel || this.isArtist || this.isOrganizer)
    ) {
  ...
  ShowPersonnel() {
    if (
      ...
      (this.isArtist || this.isPersonnel || this.isOrganizer)
    ) {
      return (
        <div>
          <Row className="tableheader mb-2">
            <Col className="border-bottom border-top">
              <b>Personell</b>
            </Col>
            <Col className="border-bottom border-top">
  ...
  emailForm() {
    if (this.artists.length != 0 && this.isOrganizer) {
      return (
        <MailForm
          hasRecipients={true}
          recipients={this.artists.concat(this.personnel)}
          toggleable={true}
        />
      );
    } else if (
      this.currentEvent.eventName &&
      (this.isPersonnel || this.isArtist) &&
      !this.isOrganizer
    ) {
      return (
        <MailForm
    ...
    if (this.isOrganizer) {
      return (
        <Row>
          <Col>
            <Button
              href={"/endre-arrangement/" + this.props.match.params.id}
              variant="primary"
            >
              Endre Arragement
            </Button>
  ...
  RenderNavbar() {
    if (authService.loggedIn()) {
      ...
      return (
        <NavLink href="/">
          <h1 className="HarmoniLogo display-sm-3 text-center m-5 text-body">
            Harmoni
          </h1>
        </NavLink>
      );
  ...
  RenderArtist() {
    if (this.artists.length !== 0) {
      return (
        <Row className="m-3">
          <Col>
            <h5>{this.artists.length > 1 ? "Artister" : "Artist"}:</h5>
          </Col>
          {this.artists.map((e) => (
            <Col>
  ...
  RenderAgeLimit() {
    if (this.currentEvent.ageLimit !== 0) {
      return (
        <h6>
          <b>Aldersgrense:</b> {this.currentEvent.ageLimit}
        </h6>
      );
    } else {
      return (
        <h6>
          <b>Aldersgrense:</b> Tillat for alle
        </h6>
      );
    }
  }
    ...
    if (this.currentEvent.imageUrl !== "") {
      return (
        <Image
          ...
          src={this.currentEvent.imageUrl}
        />
      );
    }
  }
