...
console.log(
  'v1 dot v2: ',
  v1.map((v1e, i) => v1e * v2[i]).reduce((sum, e) => sum + e)
); //32
...
console.log(
  ...
  v.map((e) => Math.sqrt(e.real * e.real + e.imag * e.imag))
);
console.log(
  'sum of v: Complex{ real: ',
  v.reduce((real, e) => real + e.real, 0) + ', imag: ' + v.reduce((imag, e) => imag + e.imag, 0) + ' }'
);
...
console.log(
  ...
  (100 * students.filter((student) => student.grade === 'C').length) / students.length
);
