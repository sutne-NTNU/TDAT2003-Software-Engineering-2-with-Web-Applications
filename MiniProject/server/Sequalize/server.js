...
app.get('/articles/categories', (req: express$Request, res: express$Response) => {
  return CategoryModel.findAll({ order: [['id', 'DESC']], limit: 20 }).then((categories) => res.send(categories));
});

//get the latest articles
app.get('/articles/#/live', (req: express$Request, res: express$Response) => {
  return ArticleModel.findAll({ order: [['id', 'DESC']], limit: 5 }).then((response) => res.send(response));
});

//Frontpage, only features important articles
app.get('/articles/#/front', (req: express$Request, res: express$Response) => {
  return ArticleModel.findAll({ order: [['id', 'DESC']], limit: 10 }).then((articles) => res.send(articles));
});

//get all articles
app.get('/articles', (req: express$Request, res: express$Response) => {
  return ArticleModel.findAll({ order: [['id', 'DESC']] }).then((students) => res.send(students));
});

//get specific article
app.get('/articles/:id', (req: express$Request, res: express$Response) => {
  return ArticleModel.findOne({ where: { id: Number(req.params.id) } }).then((student) =>
...
app.get('/articles/:id/comments', (req: express$Request, res: express$Response) => {
  return CommentModel.findAll({ where: { article_id: Number(req.params.id) } }).then((comments) => res.send(comments));
});
...
app.get('/articles/categories/:category_id', (req: express$Request, res: express$Response) => {
  return ArticleModel.findAll({ where: { category_id: Number(req.params.category_id) } }).then((articles) =>
    res.send(articles)
  );
});
...
app.delete('/articles/:id', (req: express$Request, res: express$Response) => {
  return ArticleModel.destroy({ where: { id: Number(req.params.id) } }).then((response) => {
    response === 1 ? res.sendStatus(200) : res.sendStatus(404);
  });
});

//delete a comment
app.delete('/articles/:id/comments/:c_id', (req: express$Request, res: express$Response) => {
  return CommentModel.destroy({ where: { id: Number(req.params.c_id) } }).then((response) => {
    response === 1 ? res.sendStatus(200) : res.sendStatus(404);
  });
});
