...
class Menu extends Component {
  ...
  render() {
    return (
      <Navbar className="custom_navbar" bg="dark" variant="dark" expand="lg" sticky="top">
        ...
        <Navbar.Collapse id="basic-navbar-nav">
          <Nav className="mr-auto">
            ...
            <NavDropdown title="Categories" id="basic-nav-dropdown">
              {shared.categories
                .filter((cat) => cat.id !== 1)
                .map((cat) => (
                  <NavDropdown.Item key={cat.id} href={'/#/articles/category/' + cat.id}>
                    {cat.name}
                  </NavDropdown.Item>
                ))}
              <NavDropdown.Divider />
          ...
          <button className="btn btn-outline-success my-2 my-sm-0" onClick={() => history.push('/submit')}>
            <span className="glyphicon glyphicon-plus" aria-hidden="true" aria-label="Like" />
          </button>
          <Form inline>
            <FormControl
              type="search"
...
class LiveFeedList extends Component {
  ...
  render() {
    ...
    return (
      <Carousel className="livefeed" pauseOnHover="true">
        <Carousel.Item>
...
class SearchArticles extends Component<{ match: { params: { search: string } } }> {
  articles: Article[] = [];

  mounted() {
    articleService
      .getSearchArticles(this.props.match.params.search)
      .then((articles) => (this.articles = articles))
      .catch((error) => {
        this.articles = [];
        console.log(error);
...
class ArticleList extends Component {
  ...
  render() {
    ...
    if (this.articles.length === 0) {
      return <EmptyResult message={'No articles found\n Maybe you could add one?'} />;
    }
    return (
      <div className="article_grid">
        {this.articles.map((article) => (
...
class ArticlePage extends Component<{ match: { params: { id: number } } }> {
    ...
    return (
      <div className="article_page">
        <div className="article_page-article">
          <div className="image">
            {this.article.image_link ? <img src={this.article.image_link} alt={this.article.title} /> : null}
          </div>
          <Row>
            <Button.Delete onClick={this.delete} />
            <Button.Edit onClick={() => history.push('/articles/' + this.article.id + '/edit')}>
              Edit article
            </Button.Edit>
            <Button.Like onClick={this.like} nr={this.article.likes}>
              {' '}
              Like{' '}
            </Button.Like>
          ...
          {this.comments.length > 0
            ? this.comments.map((comment) => (
                ...
                >
                  ...
                  <Button.Like nr={comment.likes} onClick={() => this.likeComment(comment)} />
                </CommentView>
              ))
            : null}
        </div>
  ...
  refreshComments() {
    articleService
      .getArticleComments(this.props.match.params.id)
  ...
  like() {
    this.article.likes++;
    articleService.updateArticle(this.article).catch((error: Error) => console.log(error.message));
  }
    ...
    return (
      <form className="input_form">
        ...
        <select
          ...
          onChange={(event: SyntheticInputEvent<HTMLInputElement>) =>
            (this.article.category_id = Number(event.target.value))
          }
        >
        ...
        <InputField.Text
          ...
          onChange={(event: SyntheticInputEvent<HTMLInputElement>) =>
            (this.article.title = (event.target.value: string))
          }
        />
        <InputField.LongText
          required
          ...
          onChange={(event: SyntheticInputEvent<HTMLInputElement>) =>
            (this.article.content = (event.target.value: string))
          }
        />
        ...
        <InputField.Image
          ...
          onChange={(event: SyntheticInputEvent<HTMLInputElement>) =>
            (this.article.image_link = (event.target.value: string))
          }
        />
        <div className="feedback">{this.feedback}</div>
    ...
    return (
      <form className="input_form">
        ...
        <InputField.LongText
          ...
          field={this.article.content}
          onChange={(event: SyntheticInputEvent<HTMLInputElement>) =>
            (this.article.content = String(event.target.value))
          }
        />
        <InputField.Image
          fieldName="Image URL"
          field={this.article.image_link}
          onChange={(event: SyntheticInputEvent<HTMLInputElement>) =>
            (this.article.image_link = String(event.target.value))
          }
        />
        <div className="feedback">{this.feedback}</div>
        <Button.Save onClick={this.save} />
...
class CategoryArticles extends Component<{ match: { params: { category_id: number } } }> {
  articles: Article[] = [];

  mounted() {
    articleService
      .getArticlesFromCategory(this.props.match.params.category_id)
      .then((articles) => (this.articles = Array.from(articles)))
      .catch((error) => {
        this.articles = [];
        console.error(error);
...
if (root) {
  ReactDOM.render(
    <HashRouter>
      <div>
        <Menu />
        <Route path="/" component={LiveFeedList} />
        <Route exact path="/" component={Home} />
        <Route exact path="/search/:search" component={SearchArticles} />
        <Route exact path="/articles" component={ArticleList} />
        <Route exact path="/articles/:id" component={ArticlePage} />
        <Route exact path="/articles/:id/edit" component={ArticleEdit} />
        <Route exact path="/articles/category/:category_id" component={CategoryArticles} />
